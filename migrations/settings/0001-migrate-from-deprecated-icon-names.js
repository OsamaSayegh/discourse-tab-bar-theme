export default function migrate(settings) {
  ...
  for (const settingName of settingNames) {
    ...
    // For tab settings, we need to handle the comma-separated format
    if (settingName.startsWith("tab_")) {
      const parts = oldSetting.split(",").map(part => part.trim());
      if (parts.length >= 2) {
        // Second part is the icon name
        parts[1] = convertIconName(parts[1]);
        settings.set(settingName, parts.join(", "));
      }
    } else if (settingName === "extend_fa5_icons") {
      // For the list setting, handle pipe-separated icons
      const newSetting = oldSetting
        .split("|")
        .map((icon) => convertIconName(icon.trim()))
        .join("|");
